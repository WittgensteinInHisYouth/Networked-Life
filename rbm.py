...
def visibleToHiddenVec(v, w):
    ...
    h = []
    for f in range(F):
        h.append(np.sum(np.multiply(v, w[:, f, :])))
    return sig(np.array(h))
...
def hiddenToVisible(h, w):
    ...
    m, F, _ = np.array(w).shape
    ret = np.zeros((m, 5))
    for i in range(m):
        ret[i, :] = sig(h.reshape(1, F) @ w[i, :, :])
    return ret
...
def getPredictedDistribution(v, w, wq):
    ...
    m, _ = np.array(v).shape
    m, F, _ = np.array(w).shape
    hidden_activation = sample(visibleToHiddenVec(v, w))
    associated_weight = np.array([wq]) #reshape it into (1, F, 5)
    negative_data = hiddenToVisible(hidden_activation, associated_weight)
    return negative_data.flatten() # return a distribution of shape (1, 5)
...
def predictRatingMean(ratingDistribution):
    ...
    ratingDistribution = np.array(ratingDistribution)
    normalized_dist = ratingDistribution/ratingDistribution.sum()
    return np.multiply(np.array([1, 2, 3, 4, 5]), normalized_dist).sum()
...
def predictRatingExp(ratingDistribution):
    ...
    return predictRatingMean(softmax(ratingDistribution))
...
def predictForUser(user, W, training, predictType="exp"):
    ...
    return np.array([predictMovieForUser(movie_ix, user, W, training, predictType=predictType) for movie_ix in range(len(W))])
